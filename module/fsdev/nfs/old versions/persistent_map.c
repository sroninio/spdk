...
void print_list(DB *data_base)
{
    ...
    for (int i = 0; i < data_base->db->header.size + 1; ++i) //
    {
        ...
        {
            printf("| %2ld ", data_base->db->entries[i - 1].key); //
        }
    } //
    printf("|\n");
    for (int i = 0; i < data_base->db->header.size + 1; ++i) //
    {
        ...
        {
            printf("| %2d ", data_base->db->entries[i - 1].value.data.num1); //
        }
    } //
    printf("|\n");
    ...
    for (int i = 0; i < data_base->db->header.size + 1; ++i) //
    {
        ...
        {
            printf("| %2d ", data_base->db->entries[i - 1].next); //
        }
    } //
    printf("|\n");
    printf("DB->HEADER->HEAD = %d || DB->HEADER->SIZE = %ld \n", data_base->db->header.head, data_base->db->header.size);
    printf("========================================================:\n");
}

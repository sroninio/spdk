...
lo_read(struct spdk_io_channel *_ch, struct spdk_fsdev_io *fsdev_io)
{
	...
	ch->nrbytes += size;
	if (!(ch->nops++ & 0xFFF)) {
		SPDK_ERRLOG("*** [BYTES_STATS] lo_read: nops=%" PRIu64 ", nread_bytes=%" PRIu64 ", thread=%s\n", ch->nops, ch->nrbytes, spdk_thread_get_name(spdk_get_thread()));
	}
#endif
...
lo_write(struct spdk_io_channel *_ch, struct spdk_fsdev_io *fsdev_io)
{
	...
	ch->nwbytes += size;
	if (!(ch->nops++ & 0xFFF)) {
		SPDK_ERRLOG("*** [BYTES_STATS] lo_write: nops=%" PRIu64 ", nwrite_bytes=%" PRIu64 ", thread=%s\n", ch->nops, ch->nwbytes, spdk_thread_get_name(spdk_get_thread()));
	}
#endif
